...
export class CustomDebugRuntime {
  ...
  public async initExecution(
    ...
    if (this._sourceFile)
      throw new Error(
        "Sources already loaded. This should only be called once after instanciation."
      );

    this._sourceFile = sourceFile;
    this.noDebug = noDebug;

    const parseResponse: ParseResponse = await this.lrProxy.parse({
      sourceFile: sourceFile,
    });
    const initResponse: InitResponse = await this.lrProxy.initExecution({
      sourceFile: sourceFile,
      ...additionalArgs,
    });
    const getBreakpointTypes: GetBreakpointTypesResponse =
      await this.lrProxy.getBreakpointTypes();
    const getRuntimeStateResponse: GetRuntimeStateResponse =
      await this.lrProxy.getRuntimeState({ sourceFile: sourceFile });

    this._breakpointManager = new CDAPBreakpointManager(
      sourceFile,
      this.lrProxy,
      parseResponse.astRoot,
      getBreakpointTypes.breakpointTypes
    );
    this.variableHandler = new VariableHandler(
      parseResponse.astRoot,
      getRuntimeStateResponse.runtimeStateRoot
    );
  ...
  public async run() {
    ...
    while (!this._isExecutionDone) {
      ...
      this._isExecutionDone = (
        await this.lrProxy.nextStep({ sourceFile: this._sourceFile })
      ).isExecutionDone;
    }
  ...
  public async nextStep() {
    ...
    const stepResponse: StepResponse = await this.lrProxy.nextStep({
  ...
  private async checkBreakpoints(): Promise<boolean> {
    this._activatedBreakpoint =
      await this._breakpointManager.checkBreakpoints();
  ...
  public async updateRuntimeState(): Promise<void> {
    const getRuntimeStateResponse: GetRuntimeStateResponse =
      await this.lrProxy.getRuntimeState({ sourceFile: this._sourceFile });
    this.variableHandler.updateRuntime(
      getRuntimeStateResponse.runtimeStateRoot
    );
  }
